classdef Library
  ...
  methods(Static)
    ...
    function []=UiPlotResultDpc(cluster,center,data,algoName,datasetName,onSave)
      clf;
      ...
      hold on
      grid on
      scatter(data(cluster<0,1),data(cluster<0,2),100,[0.6,0.6,0.6],'x');
      for p=1:NC
        scatter(data(cluster==p,1),data(cluster==p,2),100,cmap(p,:),'.');
      end
      scatter(data(center,1),data(center,2),75,cmap,'Pentagram','Filled','MarkerEdgeColor','Black');
      pbaspect([1,1,1]);
      hold off
      title(sprintf('%s on %s',algoName,datasetName));xlabel('X');ylabel('Y');zlabel('\rho')
      %Whether save figure
      if onSave
        saveas(figure(1),sprintf('..\\..\\doc\\%s\\%s',algoName,datasetName),'fig');
        saveas(figure(1),sprintf('..\\..\\doc\\%s\\%s',algoName,datasetName),'epsc');
      end
      disp('Done!');
    end
    
    function []=UiPlotResultKmeans(cluster,center,data,datasetName,onSave)
      clf;
      algoName='K-Means';
      %Handle high dimension data
      if size(data,2)>2
        rng default
        data=tsne([data;center]);
        center=data(end-15:end,:);
        data=data(1:end-16,:);
      end
      %Get colormap for plotting below
      NC=length(center);
      ...
      scatter(center(:,1),center(:,2),75,cmap,'Pentagram','Filled','MarkerEdgeColor','Black');
      pbaspect([1,1,1]);
      hold off
      title(sprintf('%s on %s',algoName,datasetName));xlabel('X');ylabel('Y');zlabel('\rho')
      %Whether save figure
      if onSave
        saveas(figure(1),sprintf('..\\..\\doc\\%s\\%s',algoName,datasetName),'fig');
        saveas(figure(1),sprintf('..\\..\\doc\\%s\\%s',algoName,datasetName),'meta');
        saveas(figure(1),sprintf('..\\..\\doc\\%s\\%s',algoName,datasetName),'svg');
      end
      disp('Done!');
    end
    
    function []=UiPlotResultDbscan(cluster,data,datasetName,onSave)
      clf;
      algoName='DBSCAN';
      %Handle high dimension data
      if size(data,2)>2
    ...
    function []=UiPlotResultAp(cluster,center,data,datasetName,onSave)
      clf;
      algoName='AP';
      %Handle high dimension data
      if size(data,2)>2
    ...
    function []=UiPlotResultGroundTruth(answer,data,datasetName,onSave)
      clf;
      %Handle high dimension data
      if size(data,2)>2
        rng default
      ...
      grid on
      for p=1:NC
        scatter(data(answer==p,1),data(answer==p,2),100,cmap(p,:),'.');
      end
      pbaspect([1,1,1]);
      hold off
      title(sprintf('%s on %s','Ground Truth',datasetName));xlabel('X');ylabel('Y');zlabel('\rho')
      %Whether save figure
      if onSave
        saveas(figure(1),sprintf('..\\..\\doc\\Ground Truth\\%s',datasetName),'fig');
        saveas(figure(1),sprintf('..\\..\\doc\\Ground Truth\\%s',datasetName),'epsc');
      end
      disp('Done!');
